...
namespace hnsw {
    ...
    struct HNSW {
        ...
        auto search_layer_cuda(const Data<>& query, int start_node_id, int ef, int l_c) {
            const int BATCH_SIZE = 1024;  // Adjust this value based on GPU memory and testing
            ...
            while (!candidates.empty()) {
                ...
                if (!batch_neighbor_ids.empty()) {
                    ...
                    // Allocate and copy batch data to GPU
                    float* d_neighbor_vectors;
                    CUDA_CHECK(cudaMalloc(&d_neighbor_vectors, batch_vectors.size() * sizeof(float)));
                    CUDA_CHECK(cudaMemcpy(d_neighbor_vectors, batch_vectors.data(), 
                                        batch_vectors.size() * sizeof(float), cudaMemcpyHostToDevice));
                    ...
                    CUDA_CHECK(cudaMemcpy(distances.data(), d_distances, 
                                        numNeighbors * sizeof(float), cudaMemcpyDeviceToHost));
