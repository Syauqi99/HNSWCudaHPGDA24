...
namespace hnsw {
    ...
    // Add new GPU-optimized structures after the existing structs
    struct GpuNode {
        float* data;
        int* neighbor_ids;
    ...
    struct HNSWVectors {
        ...
        GpuData d_dataset;
        vector<GpuNode> d_nodes;

        HNSWVectors(int m, int ef_construction = 64, bool extend_candidates = false, bool keep_pruned_connections = true) :
            ...
            // Initialize GPU dataset structure
            d_dataset = {nullptr, 0, 0};
        }

        ~HNSWVectors() {
            // Cleanup GPU resources
            if (d_dataset.vectors) {
                CUDA_CHECK(cudaFree(d_dataset.vectors));
            }
            
            // Add cleanup for d_nodes
            for (auto& node : d_nodes) {
                if (node.data) CUDA_CHECK(cudaFree(node.data));
                if (node.neighbor_ids) CUDA_CHECK(cudaFree(node.neighbor_ids));
                if (node.neighbor_distances) CUDA_CHECK(cudaFree(node.neighbor_distances));
            }
        ...
        void build(const Dataset<>& dataset_) {
            ...
            int dim = dataset[0].x.size();
            d_dataset.dimensions = dim;
            d_dataset.num_vectors = dataset.size();

            // Copy all dataset vectors to GPU memory in one go
            CUDA_CHECK(cudaMalloc(&d_dataset.vectors, dataset.size() * dim * sizeof(float)));
            CUDA_CHECK(cudaMemcpy(d_dataset.vectors, dataset[0].x.data(), 
                                dataset.size() * dim * sizeof(float), 
